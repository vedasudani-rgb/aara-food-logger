...
- **Context note for Megha:** Optional free-text `context_note` field on `MealEntry`. ConfirmationCard shows an "Add context" input below the chip list (placeholder: "Anything worth noting?") for new logs. `MealDetailSheet` also shows the same input for existing entries — always visible, auto-saves on blur via `onUpdateContextNote` prop (no separate Save needed). Surfaced in coach `DayBreakdown` as italic muted text.
